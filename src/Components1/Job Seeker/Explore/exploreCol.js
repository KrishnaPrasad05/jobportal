...
function ExploreCollections(){
  ...
  const users = useSelector((state) => state.data.users.filter(user => user.compType === "Corporation").slice(0, 12));
  const users1 = useSelector((state) => state.data.users.filter(user => user.compType === "Retails").slice(0, 12));
  const users2 = useSelector((state) => state.data.users.filter(user => user.compType === "SMEs").slice(0, 12));
  const users3 = useSelector((state) => state.data.users.filter(user => user.compType === "Finance").slice(0, 12));
  const users4 = useSelector((state) => state.data.users.filter(user => user.compType === "Healthcare").slice(0, 12));
  ...
  return (
    ...
    <nav aria-label="breadcrumb " style={{backgroundColor: '#E6E7FF',paddingLeft: '10px',top:'3.8rem',fontWeight: 500,fontSize: '16px',position:'fixed',width:'100%',zIndex:1000,height:'30px'}}>
                <ol class="breadcrumb p-1">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">Explore Collections</li>
                </ol>
              </nav>
    <style>{`
    #a .nav-link.active {
  ...
  `}</style>
      <h1 className="head text-center" style={{color:'#2B358C',marginTop:'6rem'}}>Explore Collections</h1>
      ...
      <Nav id="a" fill variant="pills" className="mb-3" style={{color:'black',paddingTop:'10px'}}>
        <Nav.Item>
          <Nav.Link eventKey="tab1">Corporations</Nav.Link>
        </Nav.Item>
        <Nav.Item>
          <Nav.Link eventKey="tab2">Retails</Nav.Link>
        </Nav.Item>
        <Nav.Item>
          <Nav.Link eventKey="tab3">SMEs</Nav.Link>
        </Nav.Item>
        <Nav.Item>
          <Nav.Link eventKey="tab4">Finance</Nav.Link>
        </Nav.Item>
        <Nav.Item>
          <Nav.Link eventKey="tab5">Healthcare</Nav.Link>
        </Nav.Item>
      </Nav>
      <Tab.Content>
