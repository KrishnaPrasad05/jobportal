...
function ExploreCategoriesCand(){
  const [key, setKey] = useState('tab1');
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []); 
  const dispatch = useDispatch();
  const users = useSelector((state) => state.resume.resumeDetails.filter(user => user.exp === "Fresher").slice(0,12));
  ...
  return (
    ...
    <nav aria-label="breadcrumb " style={{backgroundColor: '#E6E7FF',paddingLeft: '10px',top:'3.8rem',fontWeight: 500,fontSize: '16px',position:'fixed',width:'100%',zIndex:1000,height:'30px'}}>
                <ol class="breadcrumb p-1">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">Freshers</li>
                </ol>
              </nav>
      <style>{`
    #a .nav-link.active {
  ...
  `}</style>
      <h1 className="hwomand text-center" style={{color:'#2B358C',marginTop:'7rem'}}>Fresh Candidates</h1>
...
export default ExploreCategoriesCand;